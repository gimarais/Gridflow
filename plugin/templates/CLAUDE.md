<!--
  GridFlow — Claude Code project instructions (ungated fallback)

  HOW TO USE: copy this content into your project's root  CLAUDE.md  (append to an
  existing one if you already have it). Claude Code auto-loads it — no plugin feature
  required. Alternatively install the plugin/ folder, which carries the same skill.
